        ...
        for i in range(self.columns):
            for j in range(self.rows):
                if old_map[i,j]==0:
                    ...
                    for u in range(-1*self.robot_radius, self.robot_radius, 1):
                        for v in range(-1*self.robot_radius, self.robot_radius, 1):
                            if(u!=0 and v!=0):
                                if (((i+u)>=0)&((i+u)<self.columns)&((j+v)>=0)&((j+v)<self.rows)):
                                    self.result_img[i+u,j+v,1]=128
                                    self.map[i+u,j+v]=256
...
def main():
    org = np.array([403.8, 526.3])
    p = 0.5
    # map_file='office.png'
    # start = (100,42)
    # goal = (190,150)

    map_file = 'gazebo2.png'
    ...
    path = (np.array(d1.path) - org) * p
    print(len(path))
    l = int(2 / p)
    path_subsample = np.append(path[::l], [path[-1]], axis=0)
    print(len(path_subsample))
