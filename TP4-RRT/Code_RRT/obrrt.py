...
class Rrt:
    ...
    # If random number is less goal sample rate, we choose goal point as a new sample point.
    # If not, we choose randomly a point in the map as a new sample point.
    def generate_random_node(self, goal_sample_rate):
        ...
        if self.utils.is_inside_obs(node) == True and np.random.random() < 0.5:
            return Node((np.random.uniform(self.env.obs_rectangle[4][0], self.env.obs_rectangle[4][0]-delta),
                np.random.uniform(self.env.obs_rectangle[4][1], self.env.obs_rectangle[4][1]-delta)))

        return node
        
    # obtain the nearest neighbor (point) of the random node(new)
    @staticmethod
    ...
    # obtain path from start node to goal node
    ...
    # calculate the the distance and angle between two nodes
    @staticmethod
