...
class RrtStar:
    ...
    # find the node near by goal that its' cost is minimum
    ...
    # calculate the cost from start node to end node
    ...
    # calculate the cost(distance) from the node to start node
    @staticmethod
    def cost(node_p):
        node = node_p
        cost = 0.0
...
def main():
    ...
    rrt_star = RrtStar(environment, x_start, x_goal, 0.5, 0.10, 20, 20000)
